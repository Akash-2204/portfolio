...
import styles from "./circularText.module.scss"; // Import SCSS module
...
const CircularText: React.FC<CircularTextProps> = ({
  ...
  return (
    <motion.div
      initial={{ rotate: 0 }}
      className={`${styles.circularText} ${className}`} // Use SCSS module
      animate={controls}
    ...
    >
      {letters.map((letter, i) => {
        ...
        return (
          <span key={i} className={styles.letter} style={{ transform, WebkitTransform: transform }}>
            {letter}
          </span>
        );
      })}
